...
4. 模块4：信任叙事模块。目标是把评价、保障、品牌可信线索、使用反馈或交付可靠感转化为视觉化的信任闭环，降低决策风险。
...
2. 信任内容必须视觉化、故事化，例如真实使用反馈感、用户评价氛围、品牌可信线索、品质检查感或交付可靠感。
3. 信任模块必须与前文风格统一，不能突然切换成完全不同的设计系统。
4. 若本次张数有限，信任模块可以压缩为 1 张；若张数更充足，可在真实反馈、品牌背书、品质检查、交付可靠感之间做区分，但仍需保持简洁，并禁止规划包装清单、附赠配件或配件展示。
