...
- 修复生图失败场景下前端轮询被持续放大，导致一分钟内大量错误请求的问题
- 修复支付回调处理中的幂等性问题，避免同一订单重复发放积分
- 修复支付回调状态更新与数据库约束不一致导致的 `502` 错误
- 修复生图接口在未登录状态下的异常请求放大，统一返回正常业务错误

### 稳定性增强

- 增强后端日志，便于区分模型请求失败、上游 `502`、支付回调失败和轮询失败
- GitHub Action 镜像构建流程改为同时推送 `10.4` 和 `latest` 双标签
